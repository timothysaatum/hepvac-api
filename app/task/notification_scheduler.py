...
from app.core.notifications import SMSService
...
class NotificationScheduler:
    ...
    async def _was_sent_recently(
        ...
        # scalar_one() is safe here — func.count() always returns exactly one row,
        # so the result is never None. This eliminates "Operator '>' not supported
        # for None" without needing an `or 0` fallback.
    ...
    async def _send_with_logging(
        ...
        async with self._semaphore:
            ...
            await self._send_sms_with_log(db, recipient, personalized)
    ...
    async def get_stats(self, db: AsyncSession, days: int = 7) -> Dict:
        ...
        # func.count() always returns one row — scalar_one() gives a guaranteed int
        ...
        # Label both columns so the select() overload resolves unambiguously.
        # The old dict(result.all()) pattern was triggering the "No overloads for
        # __init__" error because the type checker couldn't resolve the two-column
        # select() overload. Iterating named rows is clearer and type-safe.
        status_result = await db.execute(
            select(
                NotificationLog.status.label("status"),
                func.count(NotificationLog.id).label("cnt"),
            )
            .where(NotificationLog.created_at >= date_from)
            .group_by(NotificationLog.status)
        )
        by_status: Dict = {row.status: row.cnt for row in status_result.all()}

        channel_result = await db.execute(
            select(
                NotificationLog.channel.label("channel"),
                func.count(NotificationLog.id).label("cnt"),
            )
            .where(NotificationLog.created_at >= date_from)
            .group_by(NotificationLog.channel)
        )
        by_channel: Dict = {row.channel: row.cnt for row in channel_result.all()}

        # total is now a guaranteed int, so division is safe
        success_count = (
            by_status.get(NotificationStatus.SENT.value, 0)
