...
class DashboardRepository:
    ...
    async def get_overview_stats(self, facility_id: Optional[uuid.UUID] = None) -> Dict[str, Any]:
        ...
        return cast(Dict[str, Any], {
            'total_patients': (patient_stats.total if patient_stats else None) or 0,
            'active_patients': (patient_stats.active if patient_stats else None) or 0,
            'pregnant_patients': (patient_stats.pregnant if patient_stats else None) or 0,
            'regular_patients': (patient_stats.regular if patient_stats else None) or 0,
            'total_vaccinations': (vacc_stats.total if vacc_stats else None) or 0,
            'vaccinations_this_month': (vacc_stats.this_month if vacc_stats else None) or 0,
            'vaccinations_this_year': (vacc_stats.this_year if vacc_stats else None) or 0,
            'total_revenue': (revenue_stats.total if revenue_stats else None) or 0,
            'revenue_this_month': (revenue_stats.this_month if revenue_stats else None) or 0,
            'revenue_this_year': (revenue_stats.this_year if revenue_stats else None) or 0,
            'outstanding_balance': outstanding_balance,
            'active_vaccine_purchases': (purchase_stats.active if purchase_stats else None) or 0,
            'completed_vaccine_purchases': (purchase_stats.completed if purchase_stats else None) or 0,
            'low_stock_vaccines': (stock_stats.low_stock if stock_stats else None) or 0,
            'total_vaccines': (stock_stats.total if stock_stats else None) or 0,
            'trusted_devices': (device_stats.trusted if device_stats else None) or 0,
            'pending_devices': (device_stats.pending if device_stats else None) or 0,
        })
    ...
    async def get_device_analytics(self) -> Dict[str, Any]:
        ...
        by_status: List[Dict[str, Any]] = [
            cast(Dict[str, Any], {'status': row.status, 'count': row.count})
        ...
        by_browser: List[Dict[str, Any]] = [
            cast(Dict[str, Any], {'browser': row.browser, 'count': row.count})
        ...
        by_os: List[Dict[str, Any]] = [
            cast(Dict[str, Any], {'os': row.os, 'count': row.count})
